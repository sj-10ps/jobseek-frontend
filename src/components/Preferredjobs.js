...
import { fetchalljobs, preferredjobs } from '../redux/fetchjobsslice';
...
const Preferredjobs = () => {
    ...
    const {loading,success,preferreddata}=useSelector(state=>state.fetchjob)
    const {appliedoading,appliedsuccess,applieddata}=useSelector(state=>state.applyjob)
    const [applied,setapplied]=useState('')
    const userid=localStorage.getItem("userid")
    useEffect(()=>{
     dispatch(preferredjobs(userid))
