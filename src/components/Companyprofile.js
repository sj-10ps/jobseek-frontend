...
const CompanyProfile = () => {
  ...
  // -----------------------------------------
  // FIXED: Prevent null/undefined crashes
  // -----------------------------------------
  const company = data || {};                     // FIXED
  const followers = followercount || 0;           // FIXED
  const followings = followingcount || 0;         // FIXED
    ...
    <div className="container d-flex gap-4 mt-2">
      ...
      <div className="col-md-8">
        ...
        <Card className="shadow" style={{ position: 'sticky', top: 0, maxWidth: '35rem' }}>
          <Card.Body>
            ...
            {/* FIXED: Safe fallbacks for all fields */}
            <p><strong>Email:</strong> {company.email || "N/A"}</p>
            <p><strong>Phone:</strong> {company.phone || "N/A"}</p>
            <p>
              <strong>Location:</strong>
              {company.location || ""}, {company.district || ""}, {company.state || ""}
            </p>
      ...
      <div className="col-md-4" style={{ top: '20px', maxHeight: '90vh', overflow: 'scroll', scrollbarWidth: 'none' }}>
        ...
        {location.value === "/followers" && (
          <div style={{ position: 'sticky', zIndex: 0, top: '70px' }}>
            <FollowersFollowing />
          </div>
        )}
